...
func _ready():
	...
	if not spawn_point:
		show_message("SpawnPoint not found.")
		return
	...
	if Global.selected_character_path != "":
		...
		if player_scene:
			...
			if camera:
				camera.current = true
				show_message("Character camera activated")
			else:
				show_message("Camera not found in character!")
		else:
			show_message("Could not load player scene from: %s" % Global.selected_character_path)
			get_tree().change_scene("res://scenes/CharacterSelect.tscn")
	else:
		show_message("No character selected - returning to character select")
		get_tree().change_scene("res://scenes/CharacterSelect.tscn")
